...
export default class A11YNav {
  ...
  private onButtonKeyDown(event: KeyboardEvent): void {
    ...
    if (event.key === 'Escape') {
      if (hasMenuOpen) {
        ...
        const closestMenuEl = control.el.closest<HTMLElement>('.a11y-nav-active');
      ...
      const sameLevelFocusables = this.focusables.filter(
        (focusable) => this.getMenuDepthFromEl(focusable) === this.getMenuDepthFromEl(control.el)
      );
      const currentIndex = sameLevelFocusables.findIndex((focusable) => focusable === control.el);
  ...
  private onBlur(event: FocusEvent): void {
    const menuContainsFocus = this.nav.contains(event.relatedTarget as HTMLElement);

    if (!menuContainsFocus && !!this.nav.querySelector<HTMLElement>('.a11y-nav-active')) {
      this.closeAllMenus();
    }
  }

  private controlFocusByKey(event: KeyboardEvent, els: HTMLElement[], currentIndex: number): void {
    switch (event.key) {
      case 'ArrowUp':
      case 'ArrowLeft':
        event.preventDefault();
  ...
  private openMenu(menu: Menu): void {
    ...
    this.menus.forEach((otherMenu) => {
      if (
        otherMenu !== menu &&
        this.getMenuDepthFromEl(otherMenu.el) === this.getMenuDepthFromEl(menu.el)
      ) {
  ...
  private closeMenu(menu: Menu): void {
    ...
    menu.el.querySelectorAll<HTMLElement>('.a11y-nav-menu').forEach((childMenuEl) => {
  ...
  private getControls(): Control[] {
    return Array.from(
      this.nav.querySelectorAll<HTMLButtonElement>('button[aria-expanded][aria-controls]')
    )
      .map((element) => {
