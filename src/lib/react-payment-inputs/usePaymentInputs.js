...
export default function usePaymentCard({
  ...
  const handleChangeCardNumber = React.useCallback(
    (props = {}) => {
      return (e) => {
        ...
        logOnDev('cardType', cardType?.type);
  ...
  const getCVCLabelProps = React.useCallback(
    ({ refKey, ...props } = {}) => ({
      noPropsText: cardType ? cardType.code.name : 'CVC',
      ...props,
    }),
    [cardType]
  );
  ...
  React.useLayoutEffect(() => {
    ...
    if (cvcField.current) {
      const cvcError = utils.validator.getCVCError(cvcField.current.value, cvcValidator, {
        errorMessages,
      });
      setInputError('cvc', cvcError);
    }
